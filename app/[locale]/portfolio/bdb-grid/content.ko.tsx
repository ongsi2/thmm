  ...
  return (
    <main>
      <CaseHero
        ...
        subtitle="관리자 목록 화면마다 렌더 루프와 페이징 계산을 손으로 쓰고 있었고, 나머지는 서버당 라이선스가 붙는 상용 그리드에 묶여 있었습니다. 기존 응답 계약을 그대로 흡수하는 자체 그리드를 만들어, 서버 코드를 한 줄도 안 고치고 80여 면을 옮겼습니다."
        meta={[
      ...
      >
        <p>
          바다봄 관리자에는 목록 화면이 수십 개 있습니다. 그런데 그리드가 하나로 통일돼 있지 않았어요. 크게
          두 갈래였습니다. 하나는 <strong>손으로 쓴 테이블</strong> — 화면마다 <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">for</code>{' '}
          ...
          조립했습니다. 다른 하나는 <strong>상용 그리드</strong> — 기능은 되지만 서버(IP)당 라이선스에 도메인
          제한이 붙고, 미갱신 환경에서는 개발 화면에 트라이얼 워터마크가 떴습니다.
        </p>
        <p>
          문제는 라인 수가 아니라 <em>버그가 생길 수 있는 표면적</em>이었습니다. 페이징 오프셋, XSS
          이스케이프, 엑셀 라벨 동기화 — 실수하기 쉬운 코드가 화면 수만큼 사본으로 존재하니까, 한 곳에서
          발견한 버그를 나머지에서도 똑같이 찾아 고쳐야 했습니다. 컬럼을 하나 추가하면 화면 코드와 엑셀
          조립 코드를 양쪽 다 고쳐야 했고, 한쪽만 고치는 사고가 실제로 났습니다.
        </p>
          ...
          items={[
            {
              ...
              detail:
                '목록 하나당 렌더 루프 + 페이지네비 마크업 + 총건수 갱신 + 엑셀 aoa 조립이 세트로 들어갔습니다. 같은 버그를 화면 수만큼 고쳐야 하는 구조.',
            },
            {
              title: 'XSS 이스케이프가 개발자 기억에 의존',
              detail:
                '문자열로 tr을 조립하니 esc() 호출을 빼먹으면 그대로 뚫립니다. 리뷰로 잡아야 하는 항목이 화면마다 반복됐습니다.',
            },
            {
              title: '상용 그리드는 서버당 라이선스 + 도메인 제한',
              detail:
                '서버를 늘리거나 도메인을 추가할 때마다 라이선스를 다시 따져야 했고, 개발 환경에서는 트라이얼 워터마크가 화면에 노출됐습니다.',
            },
            {
              title: '정렬·필터·엑셀이 화면마다 제각각',
              detail:
                '어떤 화면은 정렬이 되고 어떤 화면은 안 됐습니다. 사용자 입장에서 "여긴 왜 안 되냐"는 문의가 반복됐지만, 손으로 붙이자니 또 사본이 늘어나는 딜레마.',
            },
          ]}
        />
      </CaseSection>
      ...
      >
        <p>
          상용 그리드를 새로 사거나 오픈소스 그리드를 얹는 선택지도 있었습니다. 그런데 둘 다 대개{' '}
          <strong>응답 포맷을 그리드에 맞추라</strong>고 요구합니다. iBATIS + JSP 레거시 위에서 컨트롤러
          수십 개의 응답 JSON을 손대는 건, 이 시스템에서 감당할 만한 리스크가 아니었습니다. 그래서 제1
          제약을 <strong>"서버 코드 0줄 수정"</strong>으로 박고 시작했습니다.
        </p>
        <p>
          먼저 관리자 목록 ajax 계약을 전수 조사해서 실제로 쓰이는 응답 형태가 몇 계열인지 셌습니다. 세
          계열이었어요. 그래서 그리드에 <strong>봉투(envelope) 프리셋</strong>이라는 얇은 어댑터 층을 두고,
          화면에서는 계열 이름만 지정하게 했습니다. 벗어나는 화면은 함수를 직접 넘기면 되고요. 이 층 하나
          덕분에 서버는 자기 응답을 그대로 내려주면 됩니다.
        </p>
        <p>
          도입에 앞서 상용·오픈소스 그리드 14종의 기능을 갭 분석해서, "우리가 실제로 쓰는 기능"과 "카탈로그에만
          있는 기능"을 갈랐습니다. 그 결과를 P0~P2 로드맵으로 만들어 정렬·페이징 같은 필수부터 채우고, 편집
          모드와 가상 스크롤은 뒤로 미뤘습니다. 처음부터 전부 만들려 했으면 못 끝냈을 겁니다.
        </p>
        <p>
          배포 방식도 레거시에 맞췄습니다. 빌드 도구 없이 JS 한 개와 CSS 한 개로 끝내고, 관리자 콘솔
          데코레이터가 이 둘을 전 화면에 전역 로드하게 했습니다. 컴포넌트를 한 번 개선하면 WAR 재빌드 없이{' '}
          <strong>모든 그리드에 동시에 반영</strong>됩니다.
        </p>
          ...
          steps={[
            {
              ...
              body: (
                <>
                  <p>
                    화면이 하는 일은 결국 "이 URL을 이 폼으로 조회해서, 이 컬럼들을 이렇게 그린다" 하나입니다.
                    ...
                    선언만 남겼습니다. 기본 렌더러는 전부 이스케이프를 거치니까 XSS가 개발자 기억에서
                    컴포넌트 계약으로 옮겨갔습니다. 엑셀 라벨도 컬럼 제목을 그대로 쓰므로 따로 동기화할 게
                    없습니다.
                  </p>
            ...
            },
            {
              ...
              body: (
                <>
                  <p>
                    관리자 목록의 ajax 응답을 전수 조사해 보니 실제로는 세 계열이었습니다.{' '}
                    ...
                    배열형, 그리고 나머지 소수. 앞의 둘은 프리셋으로 두고 나머지는 함수를 직접 넘기게 했습니다.
                    "봉투 해석"과 "데이터 전처리"는 일부러 다른 훅으로 갈랐습니다 — 섞어 두면 응답 형태가
            ...
            },
            {
              ...
              body: (
                <>
                  <p>
                    실무에서 실제로 터지는 건 화려한 기능이 아니라 이런 것들입니다. 검색 버튼을 빠르게 두 번
                    누르면 먼저 보낸 요청이 나중에 도착해 화면을 덮습니다. 요청마다 시퀀스를 매겨서 최신이
                    아니면 응답을 버리도록 했습니다. 같은 맥락으로, 새 데이터가 들어오면 편집 상태와 정렬
                    기준을 초기화합니다 — 클라이언트 페이징의 페이지 전환은 ajax를 타지 않으므로 그때는
                    편집 상태를 유지하고요.
                  </p>
            ...
            },
            {
              ...
              body: (
                <p>
                  목록만 대체해서는 상용 그리드를 못 걷어냅니다. 남는 건 항상 편집 화면이거든요. 그래서
                  인라인 에디터, C/U/D dirty 추적, 일괄 저장 payload 파생, Undo/Redo, 엑셀 붙여넣기, 틀고정,
                  컬럼 표시/숨김, 그룹핑·트리까지 컴포넌트 안으로 넣었습니다. 상용 제품에서는 대개 상위
                  라이선스 티어에 있는 기능들입니다. 여기까지 오고 나서야 편집 화면들을 옮길 수 있었습니다.
                </p>
              ),
            },
            {
              ...
              body: (
                <p>
                  공공 사이트라 웹 접근성이 통과 조건입니다. 읽기 전용 목록은 굳이 위젯화하지 않고{' '}
                  <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">
                    caption
                  </code>{' '}
                  +{' '}
                  ...
                  + 로빙 tabindex로 갔습니다. 정렬 상태는 <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">aria-sort</code>,
                  ...
                  안내합니다. 전부 위젯으로 만드는 게 더 그럴싸해 보이지만, 읽기 목록에서는 오히려 손해였습니다.
                </p>
              ),
            },
            {
              ...
              body: (
                <p>
                  이 컴포넌트는 데코레이터를 통해 전 관리자 화면에 로드됩니다. 즉 컴포넌트 버그 하나가 전
                  화면 장애입니다. 그래서 상설 스펙 러너를 붙여 어서션을 쌓고, 실브라우저로 화면을 순회하며
                  클릭하는 배터리를 따로 돌렸습니다. 버전을 크게 올린 뒤에도 배터리 회귀 0을 확인하고 나서
                  다음 전환 배치로 넘어가는 식으로 진행했습니다. 대량 전환은 Claude Code 병렬 에이전트로
                  배치를 나눠 돌리고, 적대적 코드리뷰를 별도 에이전트로 붙여 교차 검증했습니다.
                </p>
              ),
            },
          ]}
          ...
          items={[
            {
              ...
              detail:
                '컨트롤러·엔드포인트·응답 JSON을 그대로 두고 화면만 옮겼습니다. 백엔드 리스크가 0이라 배치를 나눠 점진적으로 밀 수 있었고, 문제가 생기면 그 화면만 되돌리면 됐습니다. 이게 없었으면 이 규모를 운영 중인 시스템에서 시도하지 못했을 겁니다.',
            },
            {
              title: '버그 표면적이 화면 수 × N에서 1로',
              detail:
                '페이징 오프셋, XSS 이스케이프, 엑셀 라벨 동기화처럼 실수 잦은 코드가 컴포넌트 한 곳으로 모였습니다. 라인 수 감소(-733줄)보다 이쪽이 실제 이득이었습니다 — 한 번 고치면 전 화면에 반영되니까요.',
            },
            {
              title: '라이선스·워터마크 의존 제거',
              detail:
                '서버 증설이나 도메인 추가 때 라이선스를 다시 따질 일이 없어졌고, 개발 환경 화면에서 트라이얼 워터마크가 사라졌습니다. 오픈소스 그리드의 방치나 상용 제품의 라이선스 정책 변경에 흔들리지 않는 통제권도 같이 얻었습니다.',
            },
            {
              title: '"전부 만들지 않기"가 핵심이었음',
              detail:
                '그리드 14종 갭 분석에서 얻은 건 기능 목록이 아니라 "우리가 안 쓰는 기능"의 목록이었습니다. 카탈로그를 다 따라갔으면 지금도 못 끝냈을 겁니다. 실제로 쓰는 것부터 P0로 채우고 나머지는 필요할 때 붙였습니다.',
            },
            {
              title: '남은 것도 정직하게',
              detail:
                '아직 미전환인 수제 표가 남아 있고, 공용 검색팝업 등 일부는 상용 그리드를 그대로 씁니다. 동일 UX라 급하지 않아 신규·수정 시점에 점진 전환하는 쪽으로 뒀습니다. 인핸서(툴팁·컬럼 리사이즈)는 미전환 표에도 자동 적용되게 해서 그 사이 격차를 줄였습니다.',
            },
          ]}
        />
      </CaseSection>
