...
export const metadata: Metadata = {
  ...
  description:
    'GPT-4o가 사주·오행 기반으로 이름을 추천하고, 이상형 월드컵으로 가족이 투표해서 최종 이름을 고르는 AI 작명 서비스. Next.js 16, Supabase, Kakao OAuth 기반 사이드 프로젝트.',
  alternates: { canonical: 'https://thmm.kr/portfolio/ohmybaby' },
};

export default function OhMyBabyCase() {
  return (
    <main>
      <CaseHero
        ...
        title="AI 작명 + 이상형 월드컵으로 가족이 함께 고르는 아기 이름"
        subtitle="작명 서비스를 몇 개 써봤는데, 결국 혼자 쓰는 도구더라고요. 이름은 가족이 같이 고르는 건데. GPT-4o가 사주·오행 맞춰서 이름을 추천하면, 이상형 월드컵으로 가족이 투표해서 최종 이름을 정하는 서비스를 만들었습니다."
        meta={[
      ...
      >
        <p>
          아기 이름을 지을 일이 생겨서 작명 서비스를 몇 개 써봤습니다. AI가 이름을 추천해주는 곳도 있고,
          전통 작명 규칙을 적용해주는 곳도 있었는데 — 다 써보고 나니까 공통적으로 아쉬운 게 있었어요.
          전부 <strong>"혼자 쓰는 도구"</strong>였습니다.
        </p>
        <p>
      ...
      >
        <p>
          구조는 단순합니다. 프로젝트를 만들어서 아기 정보를 입력하면 GPT-4o가 이름을 추천하고,
          그 후보들로 이상형 월드컵을 열어서 가족한테 링크를 공유하면 각자 투표합니다.
          투표 결과가 모이면 가족이 가장 좋아하는 이름이 자연스럽게 올라오는 구조예요.
        </p>
        ...
        <div className="grid sm:grid-cols-3 gap-3 mt-8">
          {[
            {
              t: 'GPT-4o를 쓴 이유',
              d: '한국 작명은 오행, 사주, 한자 지식이 필요한 도메인. 프롬프트로 규칙을 주입하기 가장 적합한 모델이었습니다.',
            },
            {
              t: '이상형 월드컵 방식',
              d: '단순 순위 매기기보다 1:1 비교가 더 재밌고, 토너먼트 구조라 자연스럽게 후보가 줄어듭니다.',
            },
            {
              t: '카카오 공유',
              d: '한국에서 가족 단톡방 = 카카오톡. 링크 하나로 투표 참여가 되니까 진입 장벽이 거의 없습니다.',
            },
          ].map((s) => (
          ...
          steps={[
            {
              ...
              body: (
                <>
                  <p>
                    한국 전통 작명에는 규칙이 꽤 많습니다. 오행(木火土金水)의 상생·상극 관계,
                    아기의 생년월일시로 계산하는 사주팔자, 각 글자에 매칭하는 한자의 실제 사용 빈도까지.
                    이걸 시스템 프롬프트에 녹여서 GPT-4o가 "진짜 작명소에서 나올 법한" 이름을
                    추천하도록 설계했습니다.
                  </p>
                  <p>
                    특히 신경 쓴 부분은 GPT가 좋아하는 한자(瑞, 睿 같은)를 걸러내는 것이었어요.
                    모델이 자주 쓰지만 실제 한국 작명에서는 거의 안 쓰는 한자들이 있거든요.
                    그런 걸 명시적으로 금지하고, 출력 포맷도 JSON으로 고정해서 파싱 안정성을 확보했습니다.
                    프롬프트 자체는 서비스 핵심 로직이라 여기에 공개하진 않지만, 접근 방향은
                    "한국 전통 작명 규칙을 규칙 목록으로 구조화 → 금지 패턴 명시 → JSON 스키마 강제"입니다.
                  </p>
                </>
              ),
            },
            {
              ...
              body: (
                <>
                  <p>
                    4/8/16/32강을 지원하는 토너먼트 브래킷을 클라이언트에서 관리합니다.
                    후보 이름들을 랜덤으로 셔플해서 대진표를 짜고, 라운드마다 1:1 비교로
                    하나를 선택하면 다음 라운드로 진행되는 구조예요.
                  </p>
                  <p>
                    카카오톡으로 공유 링크를 보내면 가족이나 친구가 바로 투표에 참여할 수 있습니다.
                    각자의 투표 결과는 Supabase에 저장되고, 프로젝트 오너는 전체 투표 현황을 볼 수 있어요.
                    무료 플랜에서는 투표 참여를 10명까지 받을 수 있습니다.
                  </p>
                </>
              ),
            },
            {
              ...
              body: (
                <>
                  <p>
                    40개 이상의 한국 성씨에 대해 한자 변형(variants)을 매핑했습니다.
                    예를 들어 '김'이라도 金, 金海 등 다양한 한자가 있을 수 있고,
                    각 한자의 획수와 오행 속성이 다릅니다. 이 데이터가 있어야
                    GPT-4o 프롬프트에 "이 성의 오행은 金이니까 상생하는 水·木 계열 한자를 써라"
                    같은 구체적인 지시를 줄 수 있었어요.
                  </p>
                  <p>
                    이름 결과 화면에서는 각 글자의 오행을 색상으로 구분해서 보여줍니다.
            ...
            },
            {
              ...
              body: (
                <>
                  <p>
                    Supabase Auth에 카카오 OAuth를 연동했습니다. 한국 사용자 대부분이 카카오 계정을
                    갖고 있으니까 로그인 허들이 낮고, 투표 링크 공유도 카카오톡 메시지로 보내면
                    자연스럽게 바이럴이 됩니다.
                  </p>
                  <p>
                    투표가 끝나면 엽서 스타일의 결과 카드가 생성되는데, 여기에 印章(도장) 씰
                    디자인을 넣었습니다. 이것도 카카오톡으로 공유할 수 있어서,
                    "우리 아기 이름 이걸로 정했어!" 하고 알리는 용도로 쓸 수 있어요.
                  </p>
                </>
              ),
            },
