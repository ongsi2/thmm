...
export const cases: CaseInfo[] = [
  {
    ...
    i18n: {
      ko: {
        ...
        summary:
          'OTT 등 외부 기관 사이트에서 바다봄 계정으로 로그인하도록 Provider를 직접 구현했다. 일회용 UUID 토큰을 DB에 저장해 다중 WAS를 지원하고, CI(개인 공통 식별자) 기반으로 양쪽 계정을 자동 매핑한다.',
  ...
  },
  {
    ...
    i18n: {
      ko: {
        ...
        summary:
          'WAS는 대전 IDC에 있는데, 망분리 정책상 그 서버에 붙을 수 있는 PC가 부산 사무실에만 있었다. 그래서 로그 한 번 보려면 사실상 부산으로 가야 하는 구조였다. 관리자 웹 안에 SSE 기반 뷰어를 만들고, 두 개 WAS 노드 로그까지 Cross-WAS 릴레이로 한 화면에서 보이도록 구성했다.',
  ...
  },
  {
    ...
    i18n: {
      ko: {
        ...
        summary:
          'Oracle + MyBatis 기반의 OTT 기술거래 플랫폼을 PostgreSQL + iBATIS 환경으로 옮겼다. 87개 URL, 34개 JSP, 80여 개 SQL과 14개 테이블을 재작성했다.',
  ...
  },
  {
    ...
    i18n: {
      ko: {
        ...
        summary:
          '관리자 메뉴를 별도창 콘솔로 바꿔야 했는데, /adm 화면 100여 개를 다시 쓸 수는 없었다. SiteMesh 데코레이터를 그 자리에서 자식용으로 개조해 기존 매핑이 자동으로 따라오게 하고, 상단바·사이드바는 부모 셸에 상주시켜 iframe 본문만 교체하는 구조로 갔다.',
  ...
  },
  {
    ...
    i18n: {
      ko: {
        ...
        summary:
          '관리자 목록 화면마다 렌더 루프와 페이징 계산을 손으로 쓰고 있었고, 남은 화면은 서버당 라이선스가 붙는 상용 그리드에 묶여 있었다. 응답 계약을 그대로 흡수하는 자체 그리드를 만들어 서버 코드 한 줄 안 고치고 80여 면을 옮겼다.',
  ...
  },
  {
    ...
    i18n: {
      ko: {
        ...
        summary:
          '전부 수동으로 하던 빌드·배포를 Jenkins + GitLab Webhook 기반으로 자동화해서, 배포 시간을 15~20분에서 4분대로 줄였다.',
        outcome: '배포 시간 80% 단축',
      },
      en: {
  ...
  },
  {
    ...
    i18n: {
      ko: {
        ...
        summary:
          'JEUS Standard에서는 세션 클러스터링 기능을 못 써서, Redis를 외부 세션 저장소로 두고 우회했다. 덕분에 WAS 순차 재기동이 가능해졌다.',
        outcome: '무중단 배포 가능',
      },
      en: {
  ...
  },
  {
    ...
    i18n: {
      ko: {
        ...
        summary:
          'WebtoB 공용 SSL을 건드리기 부담스러워서, 앞단에 Nginx를 세우고 거기서 TLS를 종단하도록 바꿨다. 기존 서비스는 영향 없이 TLS 1.3으로 올렸다.',
  ...
  },
  {
    ...
    i18n: {
      ko: {
        ...
        summary:
          '아내가 입소한 조리원의 홈페이지가 오래돼 보여서 직접 UI 샘플을 만들어 제안했다. 192프레임 스크롤 애니메이션, 카카오맵, SEO까지 갖춘 Astro 정적 사이트를 제작해서 실제 운영 도메인으로 배포했다.',
  ...
  },
  {
    ...
    i18n: {
      ko: {
        ...
        summary:
          '기존 작명 서비스가 혼자 쓰는 구조여서, 가족이 같이 참여할 수 있는 방식을 만들었다. GPT-4o가 사주·오행 맞춰 이름을 추천하면, 이상형 월드컵으로 가족이 투표해서 최종 이름을 고른다.',
