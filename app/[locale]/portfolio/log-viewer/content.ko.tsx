...
export default function LogViewerContentKo() {
  return (
    <main>
      <CaseHero
        ...
        subtitle="WAS는 대전 IDC에 있는데, 망분리 정책상 그 서버에 붙을 수 있는 PC가 부산 사무실에만 있었다. 그래서 로그 한 번 보려면 사실상 부산까지 가야 하는 구조였다. 웹에서 실시간으로 WAS 로그를 볼 수 있게 뷰어를 만들었고, 두 개 노드에 분산된 로그까지 한 화면에서 보이도록 Cross-WAS 릴레이를 붙였다."
        meta={[
      ...
      >
        <p>
          WAS는 <strong>대전 IDC</strong>에 있다. 그 서버에 붙을 수 있는 PC가 망분리 정책상{' '}
          <strong>부산 사무실 한 곳</strong>으로 한정돼 있었다. 그러니까 "부산 사무실까지 이동해서 → 지정된 PC에
          앉아서 → 거기서 대전 WAS에 SSH" 하는 경로가 공식적으로 유일한 접근 방법이었다. 서울에서 작업하다 로그를
          봐야 하면 선택지가 둘 — 부산까지 다녀오거나, 부산에 있는 사람에게 "지금 이 시점 로그 좀 찍어서 보내주세요"
          하고 부탁하거나. 장애라도 나면 분 단위로 의사결정이 필요한데, 그 사이에 "부산 PC 앞에 누구 있나요"
          메신저가 먼저 돌아가야 하는 상황이었다.
        </p>
        <p>
          거기에 WAS가 두 노드로 나뉘어 있어서, 같은 시간대 장애의 원인을 추적하려면 두 서버의 로그를 교차로 봐야
          했다. 부산 PC에서 SSH 창 두 개 열고 대전 WAS 1/2를 번갈아 확인하는 게 기본 루틴이었고, 서울에서
          원격으로 부탁하는 흐름이 반복됐다.
        </p>
        <p>
          처음엔 단순 폴링으로 tail을 흉내내는 뷰어를 검토했는데, 이번에는 <em>로그 뷰어 자체가 access 로그를
          엄청나게 쌓아</em> 원래 보려던 로그를 덮는 역설이 생겼다. 전체 로그 파일을 매 요청마다 통째로
          읽기에는 용량도 너무 컸고.
        </p>
        <InsightList
          items={[
            {
              ...
              detail:
                '망 분리 정책상 서울·대전에서 바로 SSH를 붙을 수 없는 구조. 로그를 보려면 부산까지 가거나 부산 담당자에게 매번 부탁해야 했다.',
            },
            {
      ...
      <CaseSection eyebrow="Approach" title="관리자 웹 안으로 끌어오기 — SSE + Cross-WAS 릴레이" accent="approach">
        <p>
          핵심 아이디어는 단순한다. 서버에 붙을 수 있는 유일한 경로는 바다봄 <strong>관리자 웹 관리 기능</strong>
          이니까, 로그 뷰어도 그 안에 만들자는 거였다. 관리자 웹은 이미 바다봄 내부 망에서 동작하고 있어 기존
          접근 경로를 그대로 쓸 수 있었고, 관리자는 서울·부산 어디서든 로그인만 되면 쓸 수 있었다.
        </p>
        <p>
          실시간성은 <strong>SSE(Server-Sent Events)</strong>로 확보했다. 폴링을 쓰면 뷰어가 오히려 로그 노이즈
          원이 되니까, 장기 연결 하나로 서버가 push하는 방식이 맞았다. 멀티 노드 문제는 <strong>Cross-WAS SSE
          릴레이</strong>로 풀었다 — 내가 접속한 노드가 WAS 1인데 WAS 2의 로그를 보고 싶으면, WAS 1이 WAS 2의
          stream.do에 SSE로 붙어 받은 이벤트를 그대로 클라이언트에 중계한다. 사용자는 노드 스위치만 누르면
          되고, 어느 쪽에 붙었는지 신경 쓸 필요가 없다.
        </p>
        <p>
          대용량 로그 파일 처리는 <strong>RandomAccessFile</strong>로 끝에서 역순으로 읽어 메모리 부담 없이 꼬리
          N줄만 뽑도록 했다. 로그 rotation이 일어나면 파일 길이가 줄어드니까, 그걸 감지해서 position을 0으로
          리셋하는 처리도 같이.
        </p>
          ...
          steps={[
              ...
              body: (
                <>
                  <p>
                    ...
                    헤더로 SSE 응답을 열고, 1초 주기로 파일 변경을 체크해 새 라인을 push한다. 오래된 좀비 연결이
                    남지 않도록 10분 타임아웃을 걸고, 30초 idle일 때는 주석 한 줄짜리 heartbeat를 흘려 중간 프록시가
                    연결을 끊지 않도록 했다. 파일 크기가 줄면 rotation으로 간주하고 position을 0으로 리셋한다.
                  </p>
                ...
                <>
                  <p>
                    대상 노드가 다른 WAS면 현재 WAS가 그 노드의 <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">
                      stream.do
                    </code>에 SSE 클라이언트로 붙어, 받는 라인을 그대로 출력 스트림에 중계한다. 핵심은{' '}
                    <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">
                      setReadTimeout(0)
                    </code>{' '}
                    — SSE 장기 연결이라 읽기 타임아웃을 무제한으로 두고, 내부 WAS 간 인증은 별도 프록시 토큰 헤더로
                    처리한다. 릴레이 쪽은 폴링이 아니라 장기 SSE 연결이라 트래픽/요청 수도 최소화된다.
                  </p>
                  <CodeBlock
                ...
                <>
                  <p>
                    파일 전체를 메모리에 올리지 않고, <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">
                      RandomAccessFile
                    </code>로 끝부분 N바이트만 읽어 역순으로 라인을 뽑는다. 레벨·키워드 필터링이 걸리면 필터 후
                    남는 줄이 요청한 수에 도달할 때까지 더 큰 윈도우(512KB)를 읽도록 자동 확장한다. 이어보기는{' '}
                    <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">
                      offset
                    </code>을 반환해 프론트에서 무한 스크롤로 연결한다.
                  </p>
            ...
            },
            {
              ...
              body: (
                <>
                  <p>
                    로그는 민감 정보를 포함할 수 있어 접근 통제를 겹쳐 두었다. 내부 WAS 간 통신은 프록시 토큰으로
                    가볍게 통과시키고, 외부 사용자는 관리자 권한 + IP 화이트리스트 + 2차 비밀번호를 모두 통과해야
                    한다. 이 중 하나라도 실패하면 바로 403/401로 차단된다.
                  </p>
          ...
          items={[
            {
              ...
              detail:
                '장애 조사를 할 때 "부산 PC 앞에 누구 있나요"를 묻지 않아도 되는 구조가 됐다. 서울에서 작업 중이어도 관리자 로그인만 하면 바로 실시간 로그를 붙여 볼 수 있어서, 대응 속도가 체감될 만큼 빨라졌다.',
            },
            {
              title: '두 노드를 한 화면에서 교차 확인',
              detail:
                'Cross-WAS 릴레이 덕분에 접속한 노드와 무관하게 대상 WAS를 고를 수 있어, "내가 어느 노드에 붙었지?"를 고민할 필요가 없어졌다. 나중에 3노드 이상으로 확장돼도 같은 패턴으로 붙일 수 있다.',
            },
            {
              title: 'SSE가 운영 관점에서도 맞는 선택이었음',
              detail:
                '폴링으로 갔으면 로그 뷰어가 access log를 끝없이 쌓아 본 로그를 덮는 역설이 생길 뻔했다. 장기 연결 한 번이면 되는 SSE의 장점을 "기능" 관점 말고 "운영 로그를 어지럽히지 않는" 관점에서 체감한 사례였다.',
            },
            {
              title: '지금은 tail-only, 이후 로드맵',
              detail:
                '중앙 로그 수집(ELK/Loki 등)이 도입되면 이 뷰어의 역할 일부는 대체될 수 있다. 다만 그때까지의 "부산 의존"을 확실히 줄였다는 점만으로도 값어치는 있었다고 본다. 중앙 수집이 들어오더라도 운영자가 WAS에 바로 붙어서 보고 싶을 때 쓸 수 있는 경로로는 그대로 남을 수 있다.',
            },
          ]}
        />
      </CaseSection>
