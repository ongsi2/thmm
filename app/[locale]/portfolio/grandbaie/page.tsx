...
export const metadata: Metadata = {
  ...
  description:
    '아내가 입소한 산후조리원의 홈페이지가 오래돼 보여서 직접 UI 샘플을 만들어 제안했습니다. 192프레임 스크롤 애니메이션, 카카오맵, SEO까지 갖춘 Astro 정적 사이트를 제작해서 grandbaie.co.kr로 운영 중인 케이스 스터디.',
  alternates: { canonical: 'https://thmm.kr/portfolio/grandbaie' },
};
...
export default function GrandbaieCase() {
  return (
    <main>
      <CaseHero
        ...
        title="산후조리원 홈페이지 리뉴얼 제안 → 제작"
        subtitle="아내가 산후조리원에 입소했을 때 홈페이지를 둘러봤는데, 시설은 프리미엄인데 웹사이트가 그 느낌을 못 따라가고 있었습니다. UI 샘플을 직접 만들어서 조리원 측에 제안했고, 실제 사이트 제작까지 이어졌습니다."
        meta={[
      ...
      >
        <p>
          ...
          그 분위기를 제대로 담고 있지 못하고 있었습니다. 디자인도 좀 오래된 느낌이었고, 모바일에서는
          레이아웃이 깨지는 곳도 더러 있었고요.
        </p>
        <p>
          말로만 "리뉴얼 하면 좋을 것 같다" 하면 와닿지 않을 거라 생각해서,{' '}
          <strong>UI 샘플을 직접 만들어서 조리원 측에 보여줬습니다</strong>. 샘플을 보고 바로 긍정적인
          반응이 와서, 그대로 실제 개발로 이어졌습니다.
        </p>
        <InsightList
          items={[
            {
      ...
      >
        <p>
          산후조리원을 고르는 예비 맘 입장에서 생각해봤습니다. 검색해서 홈페이지를 열었을 때
          첫인상이 &quot;여기 괜찮아 보인다&quot;여야 방문 상담으로 이어집니다. 그래서 <strong>호텔 웹사이트
          수준의 프리미엄 느낌</strong>을 컨셉으로 잡았습니다.
        </p>
        <p>
          컬러 팔레트는 cream/espresso/gold 조합으로 따뜻하면서도 고급스러운 톤을 만들었고,
          Pretendard 폰트로 깔끔한 타이포그래피를 유지했습니다. 전체적으로 &quot;이 조리원, 꼼꼼하게
          관리하는 곳이구나&quot;라는 신뢰감이 느껴지는 방향으로 디자인했습니다.
        </p>
          ...
          steps={[
            {
              ...
              body: (
                <p>
                  산후조리원 홈페이지에는 CMS도 로그인도 필요 없습니다. 콘텐츠가 바뀔 일이 거의 없는
                  정적 사이트인 만큼, <strong>빌드 타임에 HTML을 다 만들어 두고 CDN에서 서빙</strong>하는
                  Astro가 딱 맞았습니다. JavaScript 번들이 거의 없어서 로딩이 빠르고, SSG 기반이라
                  SEO도 자연스럽게 좋습니다. Tailwind CSS 4와 조합해서 디자인 시스템 없이도 일관된
                  스타일을 빠르게 잡았습니다.
                </p>
              ),
            },
            {
              ...
              body: (
                <>
                  <p>
                    히어로 섹션에서 스크롤하면 조리원 내부를 걸어 다니는 듯한 느낌을 주고 싶었습니다.
                    192개의 WebP 프레임을 미리 로드해 두고, 스크롤 진행률에 따라 Canvas에 프레임을
                    그리는 방식입니다. 단순히 프레임을 바꾸기만 하면 뚝뚝 끊기는데, <strong>lerp 보간</strong>을
                    걸어서 현재 프레임이 목표 프레임을 부드럽게 쫓아가도록 했습니다.
                    cover-fit 로직으로 어떤 해상도에서든 Canvas가 화면을 꽉 채우게 처리했고요.
                  </p>
            ...
            },
            {
              ...
              body: (
                <p>
                  ...
                  지도가 안 뜨면 난감하니까 <strong>fallback UI</strong>를 같이 만들었습니다. 지도 로드에
                  실패하면 주소 텍스트와 카카오맵 링크 버튼이 대신 보여서, 사용자가 어떤 상황에서든
                  위치 정보를 확인할 수 있도록 했습니다.
                </p>
              ),
            },
            {
              ...
              body: (
                <p>
                  산후조리원은 지역 검색이 핵심입니다. <strong>Schema.org JSON-LD로
                  MedicalBusiness 타입</strong> 구조화 데이터를 넣어서 검색엔진이 업종·주소·전화번호를
                  제대로 파악하도록 했고, OG 태그로 카카오톡이나 SNS 공유 시 미리보기도 깔끔하게
                  나오도록 세팅했습니다. sitemap.xml 자동 생성, GA4 연동, 네이버·구글 Search Console
                  인증까지 기본적인 SEO 인프라를 전부 갖췄습니다.
                </p>
              ),
            },
          ]}
