  ...
  return (
    <main>
      <CaseHero
        ...
        subtitle="관리자 메뉴를 별도창 콘솔로 바꿔야 했는데, 사이드바 클릭마다 상단바와 무거운 리소스가 통째로 다시 그려졌습니다. 그렇다고 /adm 화면 100여 개를 재작성할 수는 없었고요. SiteMesh 데코레이터를 그 자리에서 자식용으로 개조해 기존 매핑이 자동으로 따라오게 하고, 셸은 부모에 상주시켜 iframe 본문만 바꾸는 구조로 갔습니다."
        meta={[
      ...
      >
        <p>
          담당자 요구는 짧았습니다 — &ldquo;관리자 메뉴는 별도창으로 열리게, 세션 타임아웃이랑 창 닫기 처리도
          ...
          페이지를 SiteMesh 데코레이터 하나로 감싸는 방식으로 올렸습니다. 화면 재작성 없이 콘솔 모양을 얻는
          데는 성공했어요.
        </p>
        <p>
          그런데 쓰다 보니 문제가 드러났습니다. 데코레이터가 <em>매 페이지를 통째로</em> 감싸는 구조라,
          사이드바에서 메뉴를 하나 누를 때마다 상단바·사이드바가 다시 그려지고 head에 걸린 리소스 수십 종이
          매번 재로드됐습니다. 관리자 화면은 그리드·차트·지도·대용량 업로더까지 얹혀 있어서, 화면 전환이
          체감될 만큼 무거웠습니다. 사이드바가 매번 새로 그려지니 아코디언 펼침 상태도 유지되지 않았고요.
        </p>
        <p>
          답은 명확했습니다. 상단바와 사이드바는 한 번만 로드해서 상주시키고 본문만 바꾸면 됩니다. 문제는{' '}
          <strong>그걸 하려면 보통 화면을 다시 써야 한다</strong>는 것이었습니다. <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">/adm</code>{' '}
          ...
          items={[
            {
              ...
              detail:
                '데코가 매 페이지를 감싸니 상단바·사이드바·head 리소스 29종이 전환마다 재로드됐습니다. 그리드·차트·지도가 얹힌 화면에서는 그대로 체감 지연이 됩니다.',
            },
            {
              title: '재작성 대상이 183개 JSP',
              detail:
                '/adm 콘텐츠 JSP 183개 중 166개가 풀페이지 구조. 본문만 떼어내도록 손보는 것 자체가 별도 프로젝트 규모였습니다.',
            },
            {
              title: '106개 매핑을 리맵하면 1줄 누락이 곧 사고',
              detail:
                'sitemesh 매핑을 새 데코로 하나씩 옮기는 방식은, 한 줄만 빠뜨려도 그 페이지가 셸 안에 셸로 뜹니다 — 상단바 2겹, 세션 타이머 2중, 무거운 JS 2회 로드.',
            },
            {
              title: '경계 접합부에 리스크가 집중',
              detail:
                'iframe을 한 겹 더 넣으면 위험한 곳은 106개 화면이 아니라 프레임 경계 몇 곳이었습니다. 벤더 업로더, 로그인/권한 이탈, 신규 URL 인가 등록, 메인 GNB 재진입.',
            },
          ]}
        />
      </CaseSection>
      ...
      >
        <p>
          핵심 판단은 여기였습니다. 106개 매핑을 새 자식 데코로 <strong>옮기지 않고</strong>, 기존
          데코레이터를 <strong>그 자리에서 자식용으로 개조</strong>했습니다. 상단바·사이드바·세션
          스크립트를 떼어내고 본문 렌더링만 남기니, 기존 106개 매핑이 자동으로 자식 데코를 가리키게 됩니다.
          누락될 매핑이라는 개념 자체가 사라졌어요. 새로 추가한 건 셸 URL 매핑 한 줄뿐입니다.
        </p>
        <p>
          그다음은 <strong>어디에 무엇을 두느냐</strong>의 문제였습니다. 부모 셸에는 상단바·사이드바와
          세션 워치독을, 자식에는 head 리소스와 본문을 뒀습니다. 흥미로운 건 세션 감시가 오히려 튼튼해졌다는
          점입니다 — 부모는 iframe 내비게이션에 재로드되지 않으니 타이머가 끊기지 않습니다. 반대로 자식에는{' '}
          <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">
            window.close()
          </code>
          나 타이머를 <em>절대 복제하지 않는 것</em>이 중요했습니다. iframe에서 <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">window.close()</code>는
          no-op이라, 복제해 두면 로그아웃 시 부모 팝업이 안 닫히는 치명적 회귀가 납니다.
        </p>
        <p>
          착수 전에 <strong>차단 조건 4건</strong>을 먼저 정의했습니다. 하나라도 PoC에서 실패하면 진행하지
          않는다는 기준이었어요. 벤더 대용량 업로더가 프레임 안에서 동작하는지, 로그인·권한 이탈 3갈래가
          ...
          자체는 0건이었고, 리스크는 전부 경계에 몰려 있었습니다.
        </p>
          ...
          steps={[
            {
              ...
              body: (
                <>
                  <p>
                    가장 위험한 선택지가 &ldquo;106개 매핑을 새 데코로 리맵&rdquo;이었습니다. 한 줄만
                    빠뜨려도 셸-in-셸이 되는데, 그 페이지는 겉보기엔 멀쩡하고 상단바만 두 겹으로 보입니다
                    — 리뷰에서 놓치기 딱 좋은 형태죠. 대신 기존 데코를 자식용으로 개조하니 매핑은 한 줄도
                    건드리지 않았고, 추가한 건 셸 URL exclude 한 줄이 전부였습니다.
                  </p>
            ...
            },
            {
              ...
              body: (
                <>
                  <p>
                    셸에서 실수하기 쉬운 게 두 가지였습니다. 하나는 사이드바 앵커의{' '}
                    <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">
                      target=&quot;adcFrame&quot;
                    </code>{' '}
                    — 하나라도 빠지면 그 링크가 팝업 최상위를 통째로 이동시켜 셸이 사라집니다. 다른 하나는{' '}
                    iframe의 기본 높이 150px 붕괴입니다. <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">
                      100vh → flex:1 + min-height:0 → height:100%
                    </code>{' '}
                    체인이 한 군데라도 끊기면 본문이 납작해집니다. 스크롤러도 부모·자식 각각 하나씩만 두어
                    이중 스크롤바를 없앴습니다.
                  </p>
            ...
            },
            {
              ...
              body: (
                <>
                  <p>
                    기존에는 서버에서 현재 URL로 활성 메뉴를 계산해 표시했습니다. 그런데 셸 구조에서는 부모
                    ...
                    고정되어 있어서, 자식이 무슨 화면을 보든 부모는 알 수가 없습니다. 그래서 자식 데코가 이미
                    ...
                    이벤트마다 읽어 사이드바를 갱신합니다. load는 매번 발화하니까 전용탭 이동이나 저장 후
                    리다이렉트, 뒤로가기까지 자동으로 재동기화됩니다. 브레드크럼은 DOM 텍스트 노드로만
                    조립해 주입 여지를 없앴습니다.
                  </p>
            ...
            },
            {
              ...
              body: (
                <p>
                  제일 까다로운 부분이었습니다. 인증 실패는 코드 경로가 세 갈래인데 각각 다른 방식으로
                  이동합니다. 미인증은 폼 제출, 인증-무권한은 <code className="font-mono text-[13px] px-1.5 py-0.5 rounded bg-[var(--color-bg-off)] border border-[var(--color-border)]">location.href</code>,
                  ...
                  , 상승 분기로 처리했습니다. 중요한 건 이 파일들이 <strong>비관리자 포함 전 사용자</strong>가
                  로드하는 공유 파일이라는 점이었습니다. 그래서 모든 수정을 프레임이 아닐 때 no-op이 되는
                  가드로 감싸, 비프레임 동작이 바이트 단위로 그대로이도록 했습니다.
                </p>
              ),
            },
            {
              ...
              body: (
                <>
                  <p>
                    ...
                    브라우저 전체에 뜹니다. 이 창을 콘솔로 뺏지 않고(로그인한 창은 메인 유지) 메인으로
                    보내면서 복원 힌트를 붙여, 메인 헤더가 콘솔 팝업을 되살리도록 했습니다.
                  </p>
                  <p>
                    여기서 사고가 두 번 났습니다. 처음엔 로그인 여부를 안 보고 무조건 바운스했더니, 인터셉터가
                    내려주는 로그인 유도 폼이 자동 제출되기 <em>전에</em> 튀어버려 로그인 동선 자체가
                    사라졌습니다. 다음엔 권한 거부 안내 화면까지 바운스시켰다가 &ldquo;콘솔 재진입 → 재거부
                    → 다시 바운스&rdquo; 순환이 생겼고요. 둘 다 예외 조건을 명시해 막았습니다.
                  </p>
            ...
            },
            {
              ...
              body: (
                <p>
                  구조 전환은 &ldquo;대충 되는 것 같다&rdquo;로 끝낼 수 없었습니다. 화면 하나가 콘솔 밖으로
                  ...
                  실브라우저로 전수 순회하며 콘솔 이탈 여부를 실측했습니다. 결과로 sitemesh 매핑 누락 한 줄,
                  통계 화면의 스크롤 추적 미발화, 액션 버튼 무스타일 등이 줄줄이 나왔습니다 — 리뷰만으로는
                  절대 안 나왔을 것들입니다. 이 순회는 Claude Code 병렬 에이전트로 구간을 나눠 돌렸습니다.
                </p>
              ),
            },
          ]}
          ...
          items={[
            {
              ...
              detail:
                '183개 JSP 중 한 개도 본문을 고치지 않았습니다. 데코를 in-place로 개조한 덕분에 106개 매핑이 그대로 따라왔고, 새로 쓴 건 셸 하나와 매핑 한 줄입니다. 레거시에서 구조를 바꿀 때는 "무엇을 새로 만드느냐"보다 "무엇을 안 건드리고 갈 수 있느냐"가 설계의 대부분이었습니다.',
            },
            {
              title: '세션 감시가 오히려 튼튼해짐',
              detail:
                '부모 셸은 iframe 내비게이션에 재로드되지 않으니 무활동 타이머가 중간에 끊기지 않습니다. 전환 전에는 페이지를 옮길 때마다 타이머가 새로 시작됐는데, 그게 없어졌습니다. 대신 자식에 window.close()나 타이머를 복제하지 않는 게 절대 조건이었습니다.',
            },
            {
              title: '리스크는 화면 수가 아니라 경계에 있었다',
              detail:
                '착수 전 판단이 "재사용 106페이지가 아니라 경계 접합부 4곳이 위험하다"였고, 실제로 사고는 전부 거기서 났습니다. 프레임 적대 코드를 전수 조사해 /adm 트리가 깨끗하다는 걸 먼저 확인한 게 진행 여부를 가른 근거였습니다.',
            },
            {
              title: '전수 스윕이 리뷰를 대체할 수 없는 결함을 잡았다',
              detail:
                '225면 실브라우저 순회에서 나온 건 매핑 누락 한 줄, 특정 화면의 스크롤 추적 미발화, 버튼 스타일 유실처럼 코드만 봐서는 안 보이는 것들이었습니다. 구조를 바꿀 때는 전수 순회를 자동화해 두는 게 결국 제일 싸게 먹혔습니다.',
            },
            {
              title: '별도창은 보안 조치가 아니다 — 그건 명확히 했다',
              detail:
                '요구가 별도창이라 그렇게 갔지만, 리서치 결과 정부 표준은 좌측 LNB 통합 콘솔이었고 별도창 자체의 보안 이득은 0입니다. 보안이 목적이면 호스트 분리가 맞습니다. 요구는 그대로 수용하되 그 사실은 문서로 남겨 두었습니다.',
            },
          ]}
        />
      </CaseSection>
